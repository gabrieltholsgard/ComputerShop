	...
	<table border=0>
		<form action=shop?action=profilechange method=post>
			...
			<tr>
				...
				<td><input type="text" name="street" value="${profile.street}"></td>
			</tr>
			<tr>
				<td>Zipcode:</td>
			...
			<tr>
				...
				<c:forEach var="next" items="${roles}">
					...
					<c:if test="${next.key == sessionScope.usertype}">
						<tr>
							<c:if test="${next.value == true}">
								<td><input type="checkbox" name="${next.key}" checked>
									${next.key}</td>
							</c:if>
							<c:if test="${next.value == false}">
								<td><input type="checkbox" name="${next.key}">
									${next.key}</td>
							</c:if>
